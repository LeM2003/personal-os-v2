...
export interface MissedReminder {
  id: string
  kind: 'task' | 'exam' | 'devoir' | 'devoir-late' | 'sub'
  icon: string
  title: string
  detail: string
}
...
const shouldRecurToday = (t: Task, todayStr: string, dayName: string): boolean => {
 ...
 */
export function computeMissedReminders({ tasks = [], examens = [], devoirs = [], subscriptions = [] }: MissedRemindersData): MissedReminder[] {
  ...
  tasks.forEach(t => {
    ...
    if (t.recurrenceTime >= nowHM) return
    missed.push({
      id: `task-${t.id}`,
      kind: 'task',
      icon: '🔔',
