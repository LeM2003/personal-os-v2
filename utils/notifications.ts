...
import type { Task, Exam, Homework, Course, UserProfile } from '@/types'
...
function wasNotifiedToday(key: string): boolean {
  try {
    const store = JSON.parse(localStorage.getItem('pos_notif_sent') || '{}') as Record<string, string>
...
export async function showNotif(title: string, body: string, tag: string): Promise<void> {
  ...
  const opts = {
    ...
    tag,
    vibrate: [100, 50, 100],  // vibrate est supporté par ServiceWorker mais absent des types TS
...
export async function checkAndNotify({ tasks = [], examens = [], devoirs = [], courses = [], profile }: NotifyData): Promise<void> {
  ...
  if (hour >= 7 && hour < 9) {
    ...
    if (!wasNotifiedToday(key)) {
      ...
      const todayCourses = courses.filter(c => (c as unknown as Record<string, string>)['jour'] === dayLabel)
      ...
      if (items.length > 0) {
        await showNotif(`Bonjour ${prenom} ☀️`, items.join(' · '), key)
        markNotified(key)
      }
    }
  }
  ...
  for (const task of tasks) {
    ...
    const isToday =
      ...
      (task.recurrence === 'weekly' && (task.recurrenceDays || []).includes(dayCapitalized))
  ...
  void currentTime // utilisé implicitement via recurrenceTime comparaison ci-dessus
}

// Boucle principale — appelée depuis AppContext
let notifInterval: ReturnType<typeof setInterval> | null = null

export function startNotificationLoop(getData: () => NotifyData): void {
