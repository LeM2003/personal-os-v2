...
interface ICSInclude {
  recurringTasks: boolean
  oneshotTasks: boolean
  examens: boolean
  devoirs: boolean
...
const toDT = (iso: string, hhmm = '00:00'): string => {
...
const addMinutes = (hhmm: string, mins = 30): string => {
...
const buildEvent = ({ uid, summary, description = '', location = '', dtStart, dtEnd, rrule, alarmMinutes = 15 }: BuildEventParams): string => {
  const lines = [
    'BEGIN:VEVENT',
    `UID:${uid}@personal-os`,
    `DTSTAMP:${nowUTC()}`,
...
const DEFAULT_INCLUDE: ICSInclude = { recurringTasks: true, oneshotTasks: true, examens: true, devoirs: true, subscriptions: true }

export function buildICS({ tasks = [], examens = [], devoirs = [], subscriptions = [] }: ICSData, include: ICSInclude = DEFAULT_INCLUDE): string {
  const events: string[] = []
...
export function countICSItems({ tasks = [], examens = [], devoirs = [], subscriptions = [] }: ICSData): ICSCounts {
  return {
...
export function downloadICS(data: ICSData, include: ICSInclude): void {
