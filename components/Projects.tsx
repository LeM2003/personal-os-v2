...
import type { Project, ProjectType, ProjectStatus, ProjectStep, Task } from '@/types'
import {
...
const blankProject = {
  name: '', objective: '', targetDate: '', type: 'projet' as ProjectType, notes: '',
}
...
function KanbanView({ projects, tasks, calcProgress, updateStatus, onEdit, del }: KanbanViewProps) {
  return (
  ...
  return (
    <div>
      ...
      : sorted.map(proj => {
        ...
        const linked = tasks.filter(t => t.project === proj.name)
        const steps = proj.steps || []
        const aiRaw = proj.aiAnalysis
        const ai = safeParse<{ score_faisabilite?: number; priorite_recommandee?: string; raison?: string; prochaines_etapes?: string[] } | null>(aiRaw, null)
