...
type Variant = 'rings' | 'arc' | 'bars' | 'grid' | 'stack' | 'offset'
type Tone = 'accent' | 'success' | 'warning' | 'alert' | 'muted'
...
export default function AbstractMark({ variant = 'rings', size = 96, tone = 'accent' }: AbstractMarkProps) {
  const color = TONE_COLOR[tone] ?? 'var(--gold)'
  const common = { width: size, height: size, viewBox: '0 0 96 96', fill: 'none', 'aria-hidden': true } as const
