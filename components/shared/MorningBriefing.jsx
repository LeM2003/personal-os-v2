...
export default function MorningBriefing({ tasks, examens, devoirs, expenses, projects, profile, streakData }) {
  ...
  useEffect(() => {
    ...
    // Afficher seulement entre 5h et 12h et si pas déjà montré aujourd'hui
  ...
  const generateBriefing = async () => {
    ...
    const monthBudget = (() => {
      try { return JSON.parse(localStorage.getItem('pos_budgets') || '{}').monthly || 0 } catch { return 0 }
    })()
    const monthKey = today.slice(0, 7)
    const monthExpenses = expenses.filter(e => (e.date || '').startsWith(monthKey)).reduce((s, e) => s + e.amount, 0)
    ...
    try {
      ...
      while (true) {
        ...
        // Supprimer les tags ACTION — le briefing ne crée pas de tâches
        setBriefing(text.replace(/\[ACTION:\w+:\{.*?\}\]/gs, '').trim())
      }
    } catch {
